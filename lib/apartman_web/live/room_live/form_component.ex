defmodule ApartmanWeb.RoomLive.FormComponent do
  ...
  def update(%{room: room} = assigns, socket) do
    changeset = Facility.change_room(room)
  ...
  def handle_event("validate", %{"room" => room_params}, socket) do
    changeset =
      socket.assigns.room
      |> Facility.change_room(room_params)
  ...
  defp save_room(socket, :edit, room_params) do
    case Facility.update_room(socket.assigns.room, room_params) do
      {:ok, _room} ->
        {:noreply,
         socket
  ...
  defp save_room(socket, :new, room_params) do
    case Facility.create_room(room_params) do
      {:ok, _room} ->
        {:noreply,
         socket
